...
- [ ] Reasoning/thinking chunks: Ollama `message.thinking`, Anthropic `thinking` blocks, OpenAI `reasoning` → `{ type: 'reasoning' }` stream chunks; today they are dropped and a thinking model can return empty `text` with `finishReason: 'length'`.
- [ ] `usage` object replaces flat token fields; remove dead types (D11); `fallbackProviders[]`
